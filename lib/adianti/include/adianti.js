...
function __adianti_url_params_to_json(url) {
    // Criar um objeto URL para manipular facilmente a string da URL
    let urlObj = new URL('https://www.madbuilder.com.br/'+url);
    
    // Criar um objeto vazio para armazenar os parâmetros
    let params = {};

    // Iterar sobre cada parâmetro e adicioná-lo ao objeto params
    for (let [key, value] of urlObj.searchParams.entries()) {
        // Verificar se a chave já existe no objeto params
        if (params.hasOwnProperty(key)) {
            // Se já existir, verificar se é um array
            if (Array.isArray(params[key])) {
                // Se for um array, adicionar o novo valor ao array existente
                params[key].push(value);
            } else {
                // Se não for um array, transformar o valor existente em um array e adicionar o novo valor
                params[key] = [params[key], value];
            }
        } else {
            // Se a chave não existir, simplesmente adicionar o valor
            params[key] = value;
        }
    }
