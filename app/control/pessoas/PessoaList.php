...
class PessoaList extends TPage
{
    ...
    public function __construct($param = null)
    {
        ...
        $criteria_tipo_cliente_nome = new TCriteria();
        ...
        $id_busca = new TEntry('id_busca');
        $tipo_cliente_nome = new TDBCombo('tipo_cliente_nome', 'moraisjr_eti', 'TipoCliente', 'id', '{nome}','nome asc' , $criteria_tipo_cliente_nome );

        $id_busca->exitOnEnter();

        $id_busca->setExitAction(new TAction([$this, 'onSearch'], ['static'=>'1']));

        $tipo_cliente_nome->setChangeAction(new TAction([$this, 'onSearch'], ['static'=>'1']));

        $tipo_cliente_id->enableSearch();
        $tipo_cliente_nome->enableSearch();
        ...
        $id_busca->setSize('100%');
        $documento->setSize('100%');
        $tipo_cliente_id->setSize('100%');
        $tipo_cliente_nome->setSize('100%');

        $row1 = $this->form->addFields([new TLabel("Id:", null, '14px', null, '100%'),$id],[new TLabel("Tipo cliente id:", null, '14px', null, '100%'),$tipo_cliente_id],[new TLabel("Nome:", null, '14px', null, '100%'),$nome]);
        ...
        $column_tipo_cliente_nome = new TDataGridColumn('tipo_cliente->nome', "Tipo cliente id", 'left');
        ...
        $action_onShow->setImage('fas:search #000000');
        ...
        $tr->add(TElement::tag('td', ''));
        $tr->add(TElement::tag('td', ''));
        $td_id_busca = TElement::tag('td', $id_busca);
        $tr->add($td_id_busca);
        $td_tipo_cliente_nome = TElement::tag('td', $tipo_cliente_nome);
        $tr->add($td_tipo_cliente_nome);
        ...
        $this->datagrid_form->addField($id_busca);
        $this->datagrid_form->addField($tipo_cliente_nome);
    ...
    public function onSearch($param = null)
    {
        ...
        if (isset($data->id_busca) AND ( (is_scalar($data->id_busca) AND $data->id_busca !== '') OR (is_array($data->id_busca) AND (!empty($data->id_busca)) )) )
        {

            $filters[] = new TFilter('id', '=', $data->id_busca);// create the filter 
        }

        if (isset($data->tipo_cliente_nome) AND ( (is_scalar($data->tipo_cliente_nome) AND $data->tipo_cliente_nome !== '') OR (is_array($data->tipo_cliente_nome) AND (!empty($data->tipo_cliente_nome)) )) )
        {

            $filters[] = new TFilter('tipo_cliente_id', '=', $data->tipo_cliente_nome);// create the filter 
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            // open a transaction with database 'moraisjr_eti'
            TTransaction::open(self::$database);
