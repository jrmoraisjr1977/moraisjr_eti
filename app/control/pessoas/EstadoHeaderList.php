...
class EstadoHeaderList extends TPage
{
    ...
    public function __construct($param = null)
    {
        ...
        $cep_ibge = new TEntry('cep_ibge');

        $id->exitOnEnter();
        $nome->exitOnEnter();
        $sigla->exitOnEnter();
        $cep_ibge->exitOnEnter();
        ...
        $cep_ibge->setExitAction(new TAction([$this, 'onSearch'], ['static'=>'1', 'target_container' => $param['target_container'] ?? null]));

        $id->setSize('100%');
        $nome->setSize('100%');
        $sigla->setSize('100%');
        $cep_ibge->setSize('100%');
        ...
        $column_cep_ibge = new TDataGridColumn('cep_ibge', "Codigo IBGE", 'left');
        ...
        $this->datagrid->addColumn($column_cep_ibge);
    ...
    public function onSearch($param = null)
    {
        ...
        if (isset($data->cep_ibge) AND ( (is_scalar($data->cep_ibge) AND $data->cep_ibge !== '') OR (is_array($data->cep_ibge) AND (!empty($data->cep_ibge)) )) )
        {

            $filters[] = new TFilter('cep_ibge', 'like', "%{$data->cep_ibge}%");// create the filter 
        }
